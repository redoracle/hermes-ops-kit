...
class GeminiImageAdapter(BaseImageAdapter):
    ...
    def generate(
        ...
        **kwargs,
        ...
        # The Google genai SDK auto-reads GOOGLE_API_KEY from the
        # environment. When it coexists with GEMINI_API_KEY the SDK
        # emits a noisy warning and may ignore the explicitly-passed
        ...
        try:
            ...
            # Classify the error for the router's fallback decision
            err_str = str(e)
            err_type = type(e).__name__
            if "429" in err_str or "RESOURCE_EXHAUSTED" in err_str:
                err_type = "RateLimited"
                ...
                err_type = "Unauthenticated"
            elif "500" in err_str or "503" in err_str or "UNAVAILABLE" in err_str:
                err_type = "ProviderUnavailable"

            # Extract a concise error line — Google SDK errors embed
            # verbose quota details that drown the actual message.
            concise = err_str
            for line in err_str.split("\n"):
                stripped = line.strip()
                if "message" in stripped.lower() and "quota" not in stripped.lower():
                    concise = stripped
                    break
            # Fall back to first line if nothing better found
            if len(concise) > 300:
                concise = (
                    err_str.split("\n")[0].strip() if "\n" in err_str else err_str[:300]
                )
