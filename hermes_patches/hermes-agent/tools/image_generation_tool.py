...
from tools.debug_helpers import DebugSession  # noqa: E402
from tools.fal_common import (  # noqa: E402
...
from tools.tool_backend_helpers import (  # noqa: E402
...
IMAGE_GENERATE_SCHEMA = {
    "name": "image_generate",
    "description": (
        "Generate or modify images. For new images, provide a detailed text prompt. "
        "To MODIFY an existing image, pass the image file path in the `image` parameter "
        "along with a prompt describing the desired changes. "
    ...
    "parameters": {
        ...
        "properties": {
            ...
            "image": {
                "type": "string",
                "description": "Optional: absolute path to an existing image file to modify/edit. When provided, the backend performs image-to-image generation, preserving the original composition while applying the changes described in the prompt. Use this when the user asks to modify, edit, enhance, or alter an existing image rather than create one from scratch.",
            },
            "image_path": {
                "type": "string",
                "description": "Alias for `image`: absolute path to an existing image file to modify/edit.",
            },
            "edit_mode": {
                "type": "string",
                "enum": ["generate", "edit_background"],
                "default": "generate",
                "description": "Use `edit_background` when the user asks to change only the background of an existing image.",
            },
            "preserve_subject": {
                "type": "boolean",
                "default": False,
                "description": "Set true when editing an existing image and the original subject/person must remain unchanged.",
            },
        },
        "required": ["prompt"],
    },
...
def _handle_image_generate(args, **kw):
    ...
    image_path = (
        ...
        or kw.get("image")
        or kw.get("image_path")
    )
    edit_mode = args.get("edit_mode") or kw.get("edit_mode")
    preserve_subject = bool(args.get("preserve_subject") or kw.get("preserve_subject"))
