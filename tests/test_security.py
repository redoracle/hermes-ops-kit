...
def test_rotation_note_contains_no_raw_secret():
    ...
    assert "sk-old-key-12345" not in content
    assert "sk-new-key-67890" not in content
    assert "sha256:" in content
    assert "last4=2345" in content or "last4=7890" in content
    assert "Raw secrets stored in note" in content


def test_rotation_note_rejects_raw_secret_injection():
    from docs.obsidian_sink import write_rotation_note  # pyright: ignore[reportMissingImports]
    ...
    try:
        write_rotation_note(
            provider="deepseek",
            status="success",
            mode="manual-new-key",  # key is raw, should be caught
            old_key=None,
            new_key=None,
            smoke_test="passed",
            dry_run=True,
        ...
        pass  # other exceptions okay here since no keys
