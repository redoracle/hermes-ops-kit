...
def run_simulator(scenario: str, **kwargs) -> int:
    ...
    if scenario == "provider-offline":
        ...
        results = builder()
        print(f"  Primary (github) → offline")
        print(f"  Utility (gemini) → offline")
        assert_routes_handle_failure(results)
        ...
        print(f"  Aux provider (gemini) → offline")
        assert_routes_handle_failure(results)
        print(f"  ✅ Routes still available via non-aux providers")

    elif scenario == "assistant-unavailable":
        results = builder("test-assistant")
        print(f"  Assistant → offline")
        assert results["_assistants"]["test-assistant"]["status"] == "offline"
        assert_assistant_not_in_providers(results)
        print(f"  ✅ Assistant offline — provider routes unaffected")

    elif scenario == "vaultwarden-locked":
        result = builder()
        print(f"  Vaultwarden → locked")
        assert result["ok"] is False
        assert result["unlocked"] is False
        print(f"  ✅ Backend locked — operations blocked correctly")
        ...
        print(f"  Anthropic cost spike")
        assert "cost_warning" in results.get("anthropic", {})
        assert_routes_handle_failure(results)
        print(f"  ✅ Cost warning raised — routes still functional")

    elif scenario == "restricted-denied":
        result = builder()
        print(f"  Restricted serving → denied")
        assert_policy_blocks_restricted(result)
        print(f"  ✅ Policy blocked restricted access")
