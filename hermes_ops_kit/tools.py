...
def ai_provider_invoke(args: dict, **kwargs) -> str:
    ...
    try:
        ...
        result = run_module("bridge", cmd, capture_output=True, text=True, timeout=120)
...
def ai_usage_metrics(args: dict, **kwargs) -> str:
    ...
    try:
        ...
        result = run_module(
            "usage_metrics_v2", cmd, capture_output=True, text=True, timeout=120
        )
        return redact(result.stdout)
    except Exception as exc:
        return json.dumps(
...
def ai_key_rotate(args: dict, **kwargs) -> str:
    ...
    try:
        ...
        result = run_module(
            "hermes_key_rotate", cmd, capture_output=True, text=True, timeout=120
        )
        return redact(result.stdout)
    except Exception as exc:
        return json.dumps(
...
def ai_secret_backend_status(args: dict, **kwargs) -> str:
    ...
    try:
        ...
        result = run_module(
            "hermes_key_rotate", cmd, capture_output=True, text=True, timeout=30
        )
        return redact(result.stdout)
    except Exception as exc:
        return json.dumps(
