...
def generate(
    ...
    **kwargs,
    ...
    while True:
        ...
        # Generation failed — record the attempt and try fallback.
        # Only hard configuration errors skip fallback; everything else
        # (rate limits, timeouts, quota, auth, upstream 5xx) triggers
        # the next route in priority order.
        error_type = result.get("error_type", "")
        error_msg = result.get("error", "")

        if error_type in ("MissingAPIKey", "ConfigError", "UnknownProvider"):
            if attempts:
                result["attempts"] = attempts
            return result
...
def healthcheck() -> dict:
    ...
    for key, cfg in routes.items():
        ...
        # Classify offline state so the user knows *why* a route is down:
        #   "no-key"   — required env var not set
        #   "no-quota" — key exists but API probe failed (quota/auth/network)
        #   "offline"  — service not running (e.g. local ComfyUI)
        if available:
            status = "ready"
        elif provider in ("local-comfyui",):
            status = "offline"
        else:
