...
def cmd_status(args: argparse.Namespace) -> int:
    ...
    if getattr(args, "json", False):
        con.print_json(
            ok_envelope(command="headroom.status", result=_public_state(state))
        )
        return 0
    ...
    con.print(
        f"  upstream: {sanitize_url_for_display(state['upstream_base_url']) or con.dim('(unresolved)')}"
    )
    if state["collisions"]:
...
def _doctor_checks(state: dict[str, Any]) -> list[tuple[str, bool, str]]:
    ...
    checks.append(
        (
            "proxy /readyz",
            healthy or not desired_on,
            "healthy"
            if healthy
            else (
                "down (desired=disabled — ok)"
                if not desired_on
                else f"down — run: hermes-ops-kit headroom up "
    ...
    checks.append(
        (
            ...
            bool(upstream),
            upstream
            or f"providers.{state['upstream_provider'] or '?'} has no base_url",
        )
    )
    ...
    collisions = state["collisions"]
    checks.append(
        ("fallbacks stay direct", not collisions, "; ".join(collisions) or "ok")
    )
    ...
    checks.append(
        (
            ...
            bool(fallbacks),
            f"{len(fallbacks)} entries"
            if fallbacks
            else "empty — no degradation path if the proxy dies",
        )
    )
    ...
    checks.append(
        (
            ...
            coherent,
            "ok" if coherent else "drift — run: hermes-ops-kit headroom reconcile",
        )
    )
    ...
    if healthy and desired_on and meta_upstream and upstream:
        ...
        checks.append(
            (
                "proxy upstream matches",
                upstream_match,
                "ok"
                if upstream_match
                else f"proxy forwards to {meta_upstream} but primary is "
...
def cmd_up(args: argparse.Namespace) -> int:
    ...
    if not upstream:
        return 1
    res = daemon.up(
        state["settings"], upstream, dry_run=getattr(args, "dry_run", False)
    )
    if getattr(args, "json", False):
...
def _print_reconcile(con: Console, res: dict, json_mode: bool, command: str) -> int:
    ...
    if res.get("upstream"):
        con.print(
            f"  upstream: {res['upstream']['provider']} "
...
def cmd_reconcile(args: argparse.Namespace) -> int:
    ...
    return _print_reconcile(
        con, res, getattr(args, "json", False), "headroom.reconcile"
    )
...
def cmd_enable(args: argparse.Namespace) -> int:
    ...
    return _print_reconcile(con, res, getattr(args, "json", False), "headroom.enable")
...
def cmd_disable(args: argparse.Namespace) -> int:
    ...
    if dry:
        res["warnings"].insert(0, "dry-run: desired state not persisted")
    return _print_reconcile(con, res, getattr(args, "json", False), "headroom.disable")
...
def cmd_export(args: argparse.Namespace) -> int:
    ...
    con.print_json(
        ok_envelope(command="headroom.export", result=_public_state(_gather_state()))
    )
    return 0
...
def handle_headroom_command(args: list[str]) -> int:
    ...
    for name, help_text, flags in (
        ...
        (
            "disable",
            "Set desired=disabled, reconcile, stop daemon",
            ("--json", "--dry-run"),
        ),
        (
            "reconcile",
            "Align config.yaml with the desired state",
            ("--json", "--dry-run"),
        ),
