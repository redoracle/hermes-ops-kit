...
Subprocess boundary preserved: each adapter stays an executable script with
``if __name__ == "__main__": run_cli(AdapterCls)`` and primes sys.path before
importing this module. The bridge contract is unchanged — validate_model
...
class OpenAICompatAdapter:
    ...
    @classmethod
    def op_chat(cls, prompt: str, model: str, max_tokens: int, system: str | None = None) -> dict:
    ...
    @classmethod
    def op_extract(cls, prompt: str, model: str, json_schema: dict, max_tokens: int) -> dict:
    ...
    @classmethod
    def op_review(cls, prompt: str, model: str, max_tokens: int, files: list | None = None) -> dict:
...
def run_cli(adapter_cls: type[OpenAICompatAdapter]) -> None:
    ...
    parser.add_argument("--schema", default=None, help="JSON schema for extract operation")
    parser.add_argument("--files", default=None, help="JSON array of {path, content} for review")
    ...
    try:
        if args.operation == "chat":
            result = adapter_cls.op_chat(args.prompt, args.model, args.max_tokens, args.system)
        elif args.operation == "extract":
            schema = json.loads(args.schema) if args.schema else {"type": "object", "properties": {}}
            result = adapter_cls.op_extract(args.prompt, args.model, schema, args.max_tokens)
        elif args.operation == "review":
            files = json.loads(args.files) if args.files else None
            result = adapter_cls.op_review(args.prompt, args.model, args.max_tokens, files)
...
class OpenAICompatRotator(BaseRotator):
    ...
    def validate_new_key(self, key: str) -> ValidationResult:
        ...
        try:
            ...
            return ValidationResult(
                valid=False, reason_class=ValidationReason.AUTH_DENIED, detail=redact(str(e)),
                http_status=401, retry_recommended=False,
            )
        except openai.RateLimitError as e:
            return ValidationResult(
                valid=False, reason_class=ValidationReason.RATE_LIMITED, detail=redact(str(e)),
                http_status=429, retry_recommended=True,
            )
        except openai.APITimeoutError as e:
            return ValidationResult(
                valid=False, reason_class=ValidationReason.TIMEOUT, detail=redact(str(e)),
                retry_recommended=True,
            )
        except openai.APIConnectionError as e:
            return ValidationResult(
                valid=False, reason_class=ValidationReason.NETWORK_ERROR, detail=redact(str(e)),
                retry_recommended=True,
            )
        except openai.InternalServerError as e:
            return ValidationResult(
                valid=False, reason_class=ValidationReason.SERVER_ERROR, detail=redact(str(e)),
                http_status=500, retry_recommended=True,
            )
        except openai.PermissionDeniedError as e:
            return ValidationResult(
                valid=False, reason_class=ValidationReason.QUOTA_OR_BILLING, detail=redact(str(e)),
                http_status=403, retry_recommended=False,
            )
        except Exception as e:
            return ValidationResult(
                valid=False, reason_class=ValidationReason.UNKNOWN, detail=redact(str(e))
            )
    ...
    def smoke_test(self) -> tuple[bool, str]:
        ...
        try:
            client = openai.OpenAI(api_key=secret.value, base_url=self._base_url(), timeout=15)
            models = client.models.list()
            chat = client.chat.completions.create(
                model=self.chat_model,
    ...
    def rotate(self, candidate_key: str | None = None) -> dict:
        ...
        if not vr.valid:
            if vr.reason_class == ValidationReason.QUOTA_OR_BILLING:
                ...
                audit_rotation_attempt(
                    provider=self.provider, status="failed", old_fp=old_fp, env_keys_updated=[]
                )
                return {
                    "ok": False,
        ...
        try:
            ...
            if backup:
                try:
                    ...
                    return {
                        "ok": False, "error": f"Env render failed: {e}",
        ...
        # 7. Audit
        audit_rotation_attempt(
            provider=self.provider, status="success", old_fp=old_fp, new_fp=new_fp,
            old_revoked=False, manual_action=True, env_keys_updated=[self.env_key],
        )
