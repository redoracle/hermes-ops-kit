...
def handle_ops_kit_command(args: list[str]) -> int:
    ...
    if subcmd == "status":
        return _run_script(script_dir, "usage_metrics_v2.py", ["--json"])
    elif subcmd == "usage":
        return _run_script(script_dir, "usage_metrics_v2.py", rest)
    elif subcmd == "rotate":
        return _run_script(script_dir, "hermes_key_rotate.py", rest)
    elif subcmd == "doctor":
        return _handle_doctor()
    elif subcmd in ("assistants", "assistant"):
        return _handle_assistants(script_dir, rest)
    elif subcmd == "mcp":
...
def _run_script(script_dir: str, script_name: str, args: list[str]) -> int:
    """Run a bridge script as a subprocess and pass through output."""
...
def _handle_doctor() -> int:
    ...
    try:
        ...
        r = subprocess.run(
            [
                sys.executable,
                os.path.join(
                    os.path.dirname(os.path.abspath(__file__)), "hermes_key_rotate.py"
                ),
                "--healthcheck",
            ],
            capture_output=True,
    ...
    print(f"\nNEXT")
    print(f"  hermes-ops-kit doctor")
    print(f"  hermes-usage --compact")
    print(f"  hermes-key-rotate --render-env")
...
def _load_hermes_env() -> dict[str, str]:
    """Parse ``~/.hermes/.env`` into a dict without polluting os.environ."""
...
def _credential_for_provider(
    ...
    for p, env_var, _prefix in _CREDENTIAL_CHECKS:
        if p == provider_lower:
            ...
            if val:
                return True, f"{env_var} set"
    return False, f"no credential found (expected env var)"
    ...
    for sk in AUX_SHORT_KEYS:
        ...
        if provider in ("auto", ""):
            _check(
                f"aux {sk}",
                False,
                f"provider='auto' → resolves to primary at runtime",
            )
...
def _handle_route_test(args: list[str]) -> int:
    ...
    if fallback_only:
        ...
        print(f"Fallback Chain Verification")
...
def _handle_audit(args: list[str]) -> int:
    ...
    try:
        from audit.ledger import tail_events, search_events, count_events  # pyright: ignore[reportMissingImports]
    except Exception as e:
    ...
    if sub == "tail":
        ...
        print(f"Usage: hermes-ops-kit audit [tail|search|export]")
        return 1


def _handle_assistants(_script_dir: str, args: list[str]) -> int:
...
def _handle_image(args: list[str]) -> int:
    ...
    """
    script_dir = os.path.dirname(os.path.abspath(__file__))
    manager = os.path.join(script_dir, "image_routes", "manager.py")

    if not os.path.exists(manager):
        print(f"Image route manager not found: {manager}")
        return 1
