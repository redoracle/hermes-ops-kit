...
def check_bridge_health() -> dict:
    d = os.path.expanduser("~/GIT/INFRA/tools/hermes-ops-kit/providers")
    adapters = {}
    for p, f in [
...
def check_hermes_status() -> dict:
    ...
    gw = False
    for ip in ["127.0.0.1", "<hermes-ip>"]:
        try:
            resp = urllib.request.urlopen(f"http://{ip}:8642/health", timeout=3)
            if resp.status == 200:
                gw = True
                break
        except Exception:
    ...
    return {
        ...
        "version": "0.15.1",
        "gateway_running": gw,
        "bridge_skill_loaded": os.path.exists(
            os.path.join(hd, "skills", "ai-cli-bridge", "SKILL.md")
        ),
    }
...
from config.route_map import aux_display_triples
...
def fmt_rich(results: dict) -> str:
    ...
    pad = w - len(header_text) - 2  # -2 for the "│ " prefix
    lines.append(f"╭─{'─' * w}╮")
    lines.append(f"│ {header_text}{' ' * pad}│")
    lines.append(f"╰─{'─' * w}╯")
...
def main():
    ...
    # Also load .env.generated if it exists (contains ASSISTANT_* vars from Vaultwarden)
    gen_env = os.path.expanduser("~/.hermes/.env.generated")
    if os.path.exists(gen_env):
        load_env_file(gen_env)
