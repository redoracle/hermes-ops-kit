...
def up(settings: dict, upstream_url: str, dry_run: bool = False) -> dict:
    ...
    if is_healthy(settings):
        ...
        if running and upstream_url and (
            running.rstrip("/") != upstream_url.rstrip("/")
        ):
            if dry_run:
                return {"ok": True, "started": False, "healthy": True,
                        "dry_run": True,
    ...
    if not binary:
        return {"ok": False, "started": False, "healthy": False,
    ...
    cmd = [
        ...
        "--memory-project-root", settings["memory_project_root"],
    ...
    if dry_run:
        ...
        return {"ok": True, "started": False, "healthy": False, "dry_run": True,
    ...
    while time.time() < deadline:
        if is_healthy(settings, timeout=1.5):
            return {"ok": True, "started": True, "healthy": True,
                    "message": f"proxy started (pid {proc.pid})"}
        if proc.poll() is not None:
            return {"ok": False, "started": True, "healthy": False,
                    "message": f"proxy exited early (rc={proc.returncode}); "
                               f"see {log_file}"}
        time.sleep(0.5)
    return {"ok": False, "started": True, "healthy": False,
            "message": f"proxy did not become healthy within timeout; "
                       f"see {log_file}"}
