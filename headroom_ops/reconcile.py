...
def reconcile(
    dry_run: bool = False, desired_override: bool | None = None
) -> dict:
...
def _restore_direct(
    ...
    if not snap_model.get("provider"):
        ...
        result["warnings"].append(
            f"no snapshot — restoring primary from first fallback "
            f"({first['provider']})"
        )
    if dry_run:
...
def _reconcile_enable(
    ...
    key_env = str(
        entry.get("api_key_env") or entry.get("key_env") or ""
    ).strip()
    ...
    if not proxied:
        ...
        snapshot = {
            ...
            "had_headroom_provider": (
                PROVIDER_NAME in (hermes_cfg.get("providers") or {})
                and not ours
            ),
            "prev_headroom_entry": (
                None if ours else copy.deepcopy(prev_entry)
            ),
            "aux": {},
        }
    else:
    ...
    for aux_key in settings["apply"].get("aux_routes") or []:
        ...
        if str(slot.get("provider", "")).strip() != provider:
            ...
            continue
        snapshot["aux"].setdefault(
            aux_key, {"base_url": slot.get("base_url", "")}
        )
